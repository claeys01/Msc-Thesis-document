...
% =====================================================================
\node[warmup, minimum width=\L{warmup}]                         (warmup)  at (0,0) {Gather $\mathcal{T}_{u,1}$};

\node[warmup, minimum width=\L{gap1},   right=0pt of warmup]   (s0) {};
\node[empty,  minimum width=\L{empty},  right=0pt of s0]       (e1) {\scriptsize $\mathcal{S}$ keeps running};

% \node[warmup, minimum width=\L{solverA},right=0pt of e1]       (sA) {};
\node[node,   minimum width=\L{nodeA},  right=0pt of e1]       (nA) {};
...
\node[warmup, minimum width=\L{retrain}, right=0pt of nC]       (sD) {Gather $\mathcal{T}_{u,2}$};
\node[warmup, minimum width=\L{gap1},   right=0pt of sD]       (sE) {};
\node[empty,  minimum width=\L{empty},  right=0pt of sE]       (e2) {\scriptsize $\mathcal{S}$ keeps running};

\node[node,   minimum width=\L{nodeA},  right=0pt of e2]       (nD) {};
...
% multi-line labels inside the wider NODE blocks
% \foreach \n/\txt in {nB/{NODE\\rollout}, nC/{Updating\\data}}{%
%   \node[font=\sffamily\small, align=center] at (\n.center) {\txt};
% }
...
\node[tick, below=2mm of e1.south east,anchor=north] {$t_{\text{handoff}}$};
...
% =====================================================================
% STAGE BOXES (above the bar) and connecting arrows
% =====================================================================
\node[stage, above=12mm of warmup.north east, anchor=south west, xshift=3mm]
        (pipe) {$\mathcal{P}$ training};
\node[stage, above=12mm of sD.north east, anchor=south west, xshift=3mm]
        (retr) {$\mathcal{P}$ Retraining};
        

% vertical brackets: end-of-warmup -> pipeline, end-of-tail-train -> retraining
\draw[arr] (warmup.north east) |- (pipe.west);
\draw[arr] (pipe.east) -| ($(nA.north west)+(0,0)$);

\draw[arr] (sD.north east) |- (retr.west);
\draw[arr] (retr.east) -| (nD.north west);
...
\begin{scope}[on background layer]
  ...
  \node[
    draw=black!50, dashed, rounded corners=4pt, fill=none,
    inner sep=6mm, draw=none,
    fit=(warmup)(tail)(pipe)(retr)(ood)
  ] (outer) {};
  ...
  \node[phase, fill=phasewarmup,
        fit=(warmup)
            (warmup.west |- topY) (warmup.east |- botY)]              (pW)  {};

  \node[phase, fill=phasetraining,
        fit=(s0)(e1)(pipe)
            (s0.west |- topY)   (e1.east  |- botY)]                   (pT)  {};

  \node[phase, fill=phasehybrid,
        fit=(nA)(sB)(nB)(sC)(nC)(ood)
            (nA.west |- topY)   (nC.east  |- botY)]                   (pH1) {};

  \node[phase, fill=phaseretrain,
        fit=(sD)(sE)(e2)(retr)
            (sD.west |- topY)   (e2.east  |- botY)]                   (pR)  {};

  \node[phase, fill=phasehybrid,
        fit=(nD)(nF)
            (nD.west |- topY)   (nF.east  |- botY)]                   (pH2) {};
\node[phase, fill=phaseretrain,
        fit=(tail)
            (tail.west |- topY)   (tail.east  |- botY)]                   (pR2) {};
\end{scope}

\foreach \panel/\txt in {%
  pW/Initial data-collection,
  pT/Training,
  pH1/Hybrid loop,
  pR/Retraining,
  pH2/Hybrid loop,

...
Reading Figure~\ref{fig: base_forces} chronologically, following the time axis as in the integrated workflow diagram \ref{fig: integrated_workflow}, the 4 stages of the acceleration workflow can also be seen in Figure~\ref{fig: base_forces}. Beginning with the warm-up phase, the hybrid and reference simulations start from a matching initial state at $t^*=0$. From there, both simulations are advanced in time using WaterLily, and instantaneous flow snapshots are saved to disk as training data for $\mathcal{P}$. Having reached $t^*_\text{train}$, the components of $\mathcal{P}$ can be trained; exact dynamics and time logging of the training procedures are reported in \ref{subsect: 4_training_dynamics}. After the training procedures, the hybrid loop \ref{alg: run_hybrid} is engaged at $t^*_\text{handoff}=20$ and the first rollouts are attempted. 
...
To investigate the influence of the  rollouts on the velocity fluctuations of $\mathcal{S}$

Having computed the mean 
...
\label{subsect: 4_training_dynamics}

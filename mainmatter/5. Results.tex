...
\begin{itemize}
  ...
  \item \textbf{Computational cost.} The end-to-end speed-up
        \begin{align}
          S_\text{loop} = \frac{T_\text{wall}^\text{ref}}{T_\text{wall}^\text{h, cfd} + T_\text{wall}^\text{h, rollout}},
          \qquad
          S_\text{eff}
          = \frac{T_\text{wall}^\text{ref}}{T_\text{wall}^\text{h, tot} + T_\text{wall}^\text{h, training}}
        \end{align}
    Where $S_\text{loop}$ is the speedup achieved through the switching between the latent rollouts and CFD iterations, and $S_\text{eff}$ is the speedup achieved of the complete integrated workflow as proposed in Figure~\ref{fig: integrated_workflow}. 
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}llc@{}}
        ...
        Integration step          & $\Delta t^*$ & $0.35 \cdot \frac{U}{L}$ \\
...
The cadence of the hybrid loop is controlled by 2 variables, namely $n_\text{switch}$ and $\Delta t_\text{pred}$. Where $n_\text{switch}$ is the number of CFD steps between rollout attempts in the hybrid loop (Algorithm~\ref{alg: run_hybrid}). It controls the stabilisation time between rollout attempts and, in effect, also controls the achievable speedup by the hybrid loop; each block of $n_\text{switch}$ CFD steps is the cost of letting the injected field relax back to a stable state before the next rollout attempt. The second variable, $\Delta t_\text{pred}$ sets the

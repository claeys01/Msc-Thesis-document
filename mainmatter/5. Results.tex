...
The values of $t^*_\text{handoff}, \; t^*_\text{train}, \; t^*_\text{update}$ were empirically chosen to display the learning progress of the ML pipeline. Where a $t^*_\text{train} = 17.7$ corresponds to roughly 4 shedding periods, as visible in Figure~\ref{fig: 256_forces}, for the discussed flow case. The difference of $2,5$ with $t^*_\text{handoff} = 20$ is subsequently used as test data for the AE training to perform a preliminary assessment of the AE performance when tasked with compressing flow snapshots outside of its initial training data. 

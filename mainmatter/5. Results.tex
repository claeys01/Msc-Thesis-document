...
\begin{itemize}
  \item \textbf{Computational cost.} The end-to-end speed-up
        ...
        \end{align}
    Where $S_\text{loop}$ is the speedup achieved by the hybrid loop (algorithm \ref{alg: run_hybrid}), and $S_\text{eff}$ is the speedup achieved by the complete integrated workflow as proposed in Figure~\ref{fig: integrated_workflow}. 
  ...
  \item \textbf{Mean flow and Reynolds stresses.} The $L_1$
        error over the fluid domain for the $\Omega$, \todo{as done in \cite{Lu2025}}
        \begin{align}
          \varepsilon_q
          = \lVert q - q_\text{ref} \rVert_{L_2(\Omega)}
          \qquad
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
\caption{Mean velocity components: $\langle u\rangle$ (top),
$\langle v\rangle$ (bottom). Columns: $\mathcal{S}$ (a,~d), $\mathcal{S}_\text{ref}$ (b,~e),
absolute difference (c,~f). Lengths scaled by $L$, velocities by $U$.}
\label{fig: base_meanflow}
\end{figure}

The time averaged velocity components are computed by sampling instantaneous flow snapshots of $\mathcal{S}$ and $\mathcal{S}_\text{ref}$ at the exact same $t^*$ using a matching sampling interval. Even if the time dependant properties of $\mathcal{S}$ are different to those of $\mathcal{S}_\text{ref}$, the time averaged flow fields should produce the same statistics. 

\autoref{fig: base_hybrid_mean_u} and \autoref{fig: base_ref_mean_u} show that both $\langle u\rangle$ have converged in time, showing smooth contours and gradients around the cylinder walls. Where the only visual difference can be seen in the wake of the cylinder, where the hybrid simulation shows to be less smooth than the reference. \autoref{fig: base_hybrid_mean_v} and \autoref{fig: base_ref_mean_v} also seem to to show similar behaviour, where the contours of the  mean spanwise flow of the reference is a bit less smooth compared to that of the reference simulation, indicating that the hybrid simulation lacks a small amount of averaging data. 
...
\begin{table}[ht]
  ...
  \begin{tabular}{l cc ccc}
    ...
    & $\langle u'u'\rangle$ & $\langle v'v'\rangle$ & $\langle u'v'\rangle$ \\
    \midrule
    MAE         & 0.009 & 0.005 & 0.006 & 0.014 & 0.004  \\
    Corr.\ coeff.\ & 0.9932 & 0.9928 & 0.9789 & 0.9791 & 0.9275 \\
    \bottomrule
  \end{tabular}
\end{table}
...
In the present hybrid setting, $N_z$ instead trades inference speed against the fidelity of the reconstructed flow statistics, as the sweep
over $N_z \in \{8, 16, 32\}$ (with $N_z = 16$ the baseline) in \autoref{fig: latent_speedup} shows. 
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{groupplot}[
      ...
      \nextgroupplot[ybar, bar width=16pt, ylabel={$S_\mathrm{loop}$}, ymin=0, ymax=2.9,
        ...
        \draw[dashed,gray] (axis cs:8,1) -- (axis cs:32,1)
          node[pos=0.5,above,font=\scriptsize\itshape,gray,fill=white,inner sep=1pt]{break-even};

      \nextgroupplot[ybar, bar width=16pt, ylabel={$S_\mathrm{eff}$}, ymin=0, ymax=0.13,
    ...
    \end{groupplot}
  \end{tikzpicture}
  \caption{Effect of the autoencoder latent dimension on hybrid solver
  acceleration for the 2D cylinder case ($Re \leq 2500$). Left: inference-loop
  speedup $S_\mathrm{loop}$; the dashed line marks break-even ($S=1$). Right:
  effective speedup $S_\mathrm{eff}$, including one-time training cost.}
  \label{fig: latent_speedup}
\end{figure}
\todo{replace bar chart}



The smallest bottleneck proves to be the best of the three models, as with $N_z=8$ the hybrid loop achieves $S_\text{loop} = 2.51$, against $1.48$ and $1.64$ for $N_z = 16$ and $32$. The acceleration performance can largely be attributed to the longer average rollout length of the smallest bottleneck, with average rollout length of $964$ CFD steps per accepted prediction, against $408$ and $421$ for $N_z = 16$ and $32$. The lower dimensional latent space presents simpler dynamics for the compact MLP $f_\phi$ to learn, which allows it to keep $\tilde{\boldsymbol{\mathcal{T}}}_z$ in distribution for longer before the KNN monitor flags. However, because of the lower amount of values that are able to contain information about the turbulent flow, it is not able to produce the same result when assessed over mean flow and RST accuracy, as displayed in \autoref{tab: latent_field_mae}.
...
\toprule
 & & {$N_z = 8$} & {$N_z = 16$ (base)} & {$N_z = 32$} \\
\cmidrule(lr){3-3} \cmidrule(lr){4-4} \cmidrule(lr){5-5}
Field & Quantity & {MAE} & {MAE} & {MAE} \\
\midrule
\multirow{2}{*}{Mean flow}
  & $\langle u \rangle$    & 0.0109 & 0.0094 & 0.0102 \\
  & $\langle v \rangle$    & 0.0078 & 0.0053 & 0.0074 \\
\addlinespace
\multirow{3}{*}{Reynolds stresses}
  & $\langle u'u' \rangle$ & 0.0100 & 0.0064 & 0.0049 \\
  & $\langle v'v' \rangle$ & 0.0274 & 0.0138 & 0.0103 \\
  & $\langle u'v' \rangle$ & 0.0065 & 0.0041 & 0.0034 \\
\bottomrule
\end{tabular}
\caption{Mean absolute error of the time-averaged velocity components and the
Reynolds-stress tensor for the latent-dimension sweep, computed over the full1
fluid domain against each run's own reference simulation $\mathcal{S}_\text{ref}$.
Velocities are scaled by $U$, so the mean-flow MAE is in units of $U$ and the
Reynolds-stress MAE in units of $U^2$.}
\label{tab: latent_field_mae}
\end{table}

This shows that the 

\subsubsection{Training Epochs}
The number of training epochs are varied, since the autoencoders' initial training consumes most of the wall time, as \autoref{tab: wall_timing_hybrid} shows. But it also controls how strict the instantaneous dynamics are learned, a

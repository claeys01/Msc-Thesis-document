...
The mean drag coefficient allows the assessment of the physically meaningful streamwise loading, whereas the mean lift coefficient vanishes due to symmetry, so the fluctuation amplitude $C_L^\text{rms}$ is compared. The comparison of the relative errors allow comparison between different hybrid simulations. These integral force metrics confirm loading but tell nothing about the errors in the wake of the simulation, which motivates the comparison metrics of the field quantities below. 
...
The number of training epochs are varied around the baseline of $500$, since the autoencoders' initial training consumes most of the wall time, as \autoref{tab: wall_timing_hybrid} shows, but it also controls how strictly the instantaneous dynamics are learned. The initial epoch count, therefore, sits directly on the trade-off between offline cost and reconstruction fidelity. 

\begin{figure}[htp]
    \centering
    \includegraphics[width=\linewidth]{figures/Results/loss_evolution_750.png}
    \caption{Loss and correlation coefficient evolution over the initial training run sampling data $0 \mapsto t^*_\text{train}$}
    \label{fig: 750_epochs}
\end{figure}

The initial training epochs are varied between 
...
% Finally, the divergence and curl weights $(\lambda_\text{div},\,\lambda_\text{curl})$ govern how strictly the reconstructed field respects the incompressibility and vorticity structure of the flow; because $\hat{\mathbf{u}}$ is re-injected into the WaterLily solver, a reconstruction that is sharp but physically inadmissible can destabilise the downstream simulation, so these weights trade reconstruction fidelity against physical consistency rather than against cost.
...
% \todo{Put autoencoder reconstruction behaviour in appendix, focus on actuall changes in autoencoder architecture on the accuracy of the hybrid simulations}
...
% \subsection{Transfer to a Lower Reynolds Number}
